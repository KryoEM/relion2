	...
	else
	{
		...
		for (size_t icom = 0; icom < commands.size(); icom++)
		{
			...
			if ((has_mpi && nr_mpi.getValue() > 1)  &&
				...
				one_command = "/usr/bin/time mpirun -n " + floatToString(nr_mpi.getValue()) + " ";
...
UnblurTbzJobWindow::UnblurTbzJobWindow() : RelionJobWindow(3, HAS_MPI, HAS_THREAD)
{
	type = PROC_UNBLURTBZ;
	...
	// read settings if hidden file exists
	read(".gui_unblurtbz", is_continue);
}

void UnblurTbzJobWindow::write(std::string fn)
{
	// Write hidden file if no name is given
	if (fn=="")
		fn=".gui_unblurtbz";
...
void UnblurTbzJobWindow::read(std::string fn, bool &_is_continue)
{
	std::ifstream fh;
...
void UnblurTbzJobWindow::toggle_new_continue(bool _is_continue)
{
	is_continue = _is_continue;
									 ...
									 std::string &final_command, bool do_makedir, int job_counter)
{
	...
	command += " -f " + floatToString(first_frame_sum.getValue());
	command += " -l " + floatToString(last_frame_sum.getValue());

	command += " -un " + fn_unblur_exe.getValue();
	command += " -sm " + fn_summovie_exe.getValue();

	if (do_dose_weighting.getValue()) {
		command += " -d=True";
		command += " -v " + floatToString(voltage.getValue());
		command += " -df " + floatToString(dose_per_frame.getValue());
	...
	return prepareFinalCommand(outputname, commands, final_command, do_makedir);
	// Add number of threads support here
}
