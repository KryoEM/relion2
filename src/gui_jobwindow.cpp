...
{
	commands.clear();
	initialisePipeline(outputname, "MotionCorrMC", job_counter);
...
// END MOTIONCORR JOB WINDOW

UnblurJobWindow::UnblurJobWindow() : RelionJobWindow(3, HAS_MPI, HAS_THREAD)
{
	type = PROC_UNBLUR;
	...
	read(".gui_unblur", is_continue);
}

void UnblurJobWindow::write(std::string fn)
{
	if (fn == "")
		fn = ".gui_unblur";
...
void UnblurJobWindow::read(std::string fn, bool &_is_continue)
{
	std::ifstream fh;
...
void UnblurJobWindow::toggle_new_continue(bool _is_continue)
{
	is_continue = _is_continue;
...
bool UnblurJobWindow::getCommands(std::string &outputname, std::vector<std::string> &commands,
		std::string &final_command, bool do_makedir, int job_counter)
{
	commands.clear();
	initialisePipeline(outputname, "MotionCorrUB", job_counter);
